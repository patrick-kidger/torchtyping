...
def _check_tensor(argname: str, value, hint):
    if not isinstance(value, hint):
        raise TypeError(f"{argname} must be {hint}, got {hint.like(value)} instead.")
...
def patch_typeguard():
    ...
    if unpatched_typeguard:
        ...
        class _CallMemo(typeguard._CallMemo):
            __slots__ = ("argname_shape_hints", "argname_value_hints")
        ...
        def check_type(*args, **kwargs):
            ...
            is_tensor_type = memo is not None and hasattr(memo, "argname_shape_hints") and inspect.isclass(expected_type) and issubclass(expected_type, TensorType) and expected_type.dims is not None
        ...
        def check_argument_types(*args, **kwargs):
            ...
            if memo is None:
                ...
                # Run it once, to find all the tensors needing extra hinting
                ...
                while len(memo.argname_shape_hints):
                    for argname, shape, hint in memo.argname_shape_hints:
                        ...
                        for dim in hint.dims:
                            if dim.size is ... and dim.name not in name_to_shape:
                                num_free_ellipsis += 1
                        if num_free_ellipsis <= 1:
                            ...
                            for reverse_dim_index, dim in enumerate(reversed(hint.dims)):
                                try:
                                    ...
                                    if dim.size is ...:
                                        if dim.name is not None:
                                            _handle_ellipsis_size(dim.name, name_to_shape, ())
                                    else:
                                        raise TypeError(
                                ...
                                if dim.name is not None:
                                    if dim.size == -1:
                                        try:
                                            lookup_size = name_to_size[dim.name]
                                        except KeyError:
                                            name_to_size[dim.name] = size
                                        else:
                                        ...
                                        _handle_ellipsis_size(
                                            dim.name, name_to_shape, clip_shape
                                        )
                        ...
                        if len(memo.argname_shape_hints):
                            ...
                            raise TypeError(f"Could not resolve the size of all `...` in arguments {names}.")
                    ...
                    for dim in hint.dims:
                        ...
                        if name is not None:
                            if size == -1:
                                ...
                                for size in name_to_shape[name]:
